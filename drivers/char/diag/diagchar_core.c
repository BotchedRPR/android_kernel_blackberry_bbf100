...
/* MODIFIED-BEGIN by hongwei.tian, 2019-12-10,BUG-8676158*/
...
/* MODIFIED-END by hongwei.tian,BUG-8676158*/
			   ...
			   unsigned int iocmd, unsigned long ioarg)
{
	...
	case DIAG_IOCTL_SWITCH_LOGGING:
		if (copy_from_user((void *)&mode_param, (void __user *)ioarg,
				   sizeof(mode_param)))
			return -EFAULT;
		diag_switch_logging_clear_mask(&mode_param, current->tgid); // MODIFIED by hongwei.tian, 2019-12-10,BUG-8676158
		mutex_lock(&driver->diagchar_mutex);
...
static int diag_user_process_raw_data(const char __user *buf, int len)
{
	...
	if (driver->mask_check) {
		if (!mask_request_validate(user_space_data +
						token_offset, len)) { // MODIFIED by hongwei.tian, 2019-09-30,BUG-8383821
