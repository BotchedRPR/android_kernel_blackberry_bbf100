...
/* MODIFIED-BEGIN by hongwei.tian, 2019-06-03,BUG-7786885*/
...
/* MODIFIED-END by hongwei.tian,BUG-7786885*/
			...
			unsigned char *dest_buf, int dest_len, int pid)
{
	...
	/* MODIFIED-BEGIN by hongwei.tian, 2019-09-30,BUG-8383821*/
	if (!src_buf || !dest_buf || dest_len <= 0 ||
 		src_len < sizeof(struct diag_build_mask_req_t)) {
 		/* MODIFIED-END by hongwei.tian,BUG-8383821*/
			...
			unsigned char *dest_buf, int dest_len, int pid)
{
	...
	mutex_unlock(&mask_info->lock);
	mutex_unlock(&driver->md_session_lock);
	if (diag_check_update(APPS_DATA, pid)) // MODIFIED by hongwei.tian, 2019-06-03,BUG-7786885
	...
	for (i = 0; i < NUM_PERIPHERALS; i++) {
		if (!diag_check_update(i, pid)) // MODIFIED by hongwei.tian, 2019-06-03,BUG-7786885
			continue;
		mutex_lock(&driver->md_session_lock);
				 ...
				 int pid)
{
	...
	for (i = 0; i < MAX_EQUIP_ID && !status; i++, mask++) {
		...
		if (mask_size > 0 && src_len >= read_len + mask_size) // MODIFIED by hongwei.tian, 2019-09-30,BUG-8383821
			    ...
			    int update_buf_len)
{
	...
	if (update_buf_len > 0) {
		...
		if (!mask_info->update_buf) {
			kfree(mask_info->ptr);
			mask_info->ptr = NULL; // MODIFIED by hongwei.tian, 2019-12-06,BUG-8663284
			return -ENOMEM;
		}
