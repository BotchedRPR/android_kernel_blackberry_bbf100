...
#else
	if (!check_caller_access_to_name(dir, &dentry->d_name, FMODE_WRITE)) {
...
#else
	if (!check_caller_access_to_name(old_dir, &old_dentry->d_name, FMODE_WRITE) ||
		!check_caller_access_to_name(new_dir, &new_dentry->d_name, FMODE_WRITE)) {
#endif /* CONFIG_BBSECURE_SDBASE */
		err = -EACCES;
		goto out_eacces;
	}
...
#else
		if (!check_caller_access_to_name(d_inode(parent), &dentry->d_name, FMODE_WRITE))
#endif /* CONFIG_BBSECURE_SDBASE */
			err = -EACCES;
		dput(parent);
	}
		 ...
		 struct kstat *stat)
{
	...
	if (!check_caller_access_to_name(d_inode(parent), &dentry->d_name)) {
#else
	if (!check_caller_access_to_name(d_inode(parent), &dentry->d_name, FMODE_READ)) {
#endif /* CONFIG_BBSECURE_SDBASE */
		dput(parent);
		return -EACCES;
	}
